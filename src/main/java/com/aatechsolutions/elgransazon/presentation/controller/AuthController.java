    ...
    public String login(@RequestParam(value = "error", required = false) String error,
        ...
        if (error != null) {
            if ("clientAttempt".equals(error)) {
                ...
                model.addAttribute("error",
                        "El límite de usuarios activos ha sido reducido. Contacta al administrador para que ajuste los empleados activos.");
            } else if ("restaurantClosed".equals(error)) {
                log.warn("Employee login blocked because the restaurant is closed");
                model.addAttribute("error", "El restaurante está cerrado. No puedes iniciar sesión en este momento.");
            } else if ("outsideShift".equals(error)) {
                log.warn("Employee login blocked because they are outside their assigned shift");
                model.addAttribute("error", "No estás en un turno laborable en este momento.");
            } else {
        ...
        if (restricted != null) {
            ...
            model.addAttribute("warning",
                    "El módulo de clientes no está disponible con tu licencia actual. Contacta al administrador para actualizar.");
        }
